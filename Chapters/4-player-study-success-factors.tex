...
\advice{Could possibly split the following into its own pair of sections? But it is somewhat related to the above, and this section would be \emph{very} short without it.}
...
\emph{TCG} refers to the Pokémon Trading Card Game, both paper and digital versions. \emph{Fighting} refers to Pokémon fighting games such as \emph{Pokémon Stadium} and \emph{Pokémon Colosseum}. \emph{Puzzle} refers to Pokémon puzzle games for consoles, such as \emph{Pokémon Puzzle Challenge} and \emph{Pokémon Shuffle}. \emph{RPG} refers to the \emph{"main series"} Pokémon Role-Playing Games for handheld devices, such as \emph{Pokémon Red/Blue} and \emph{Pokémon X/Y}. \emph{Mobile} refers to other Pokémon games for mobile phones besides Pokémon GO, and \emph{Snap} refers to \emph{Pokémon Snap}, a popular game for Nintendo 64 consoles. \emph{None} are respondents who had no previous experience with Pokémon games.
...
\todo{PoGo vs other Pokémon: Combat is very simple (mostly negative, but positive for casual gamers), no connection with your Pokémon because you don't level them (very negative!), very random because you can't choose moves (negative), get you out and walking (positive, but negative because you can't play at home), social (positive), immersive because of the real world (positive), portable because no extra device (positive), easy to get into (positive), no story (negative), free to play with no purchase required (positive), game gets updates post-release (positive)}

\todo{PoGo vs other mobile: Social (positive), immersive/real world (positive), can't play anytime/anywhere (negative), needs focus to play (negative), not much feeling of progression (negative), no ads and no purchase required (positive), pogo can be played for longer sessions (positive), has pokemon (positive)}

\todo{PoGo vs Ingress: less complex (positive and negative), more interesting theme (positive), lack of goals makes it not really a game (negative), pogo still provides something to do when there are no stops/gyms (positive), more friendly (positive), less competitive (negative with some positive), bigger player base (positive), less immersion with lack of videos and story (negative), more reasons to play on foot (positive)}

\todo{PoGo vs Geocaching: easier to find something (positive and negative), provides a purpose for the things you find (positive), no extra tools needed (positive), provides something to do between locations (positive), does not require planning to play and can be aborted at any time (positive), gamification of regular walking (positive)}

\todo{PoGo vs other AR: more immersive than Zombies Run/The Walk (positive), can be combined with podcasts/music (positive), can be played at own pace (positive), better for use in cities (positive)}

\todo{PoGo vs other location-based/exercise: bigger player base/more competition (positive), more social (positive), lack of story (negative), better theme than zombies (subjectively positive), requires less focus than The Walk (positive)}

\section{Analysis of Results for Most Used Features and Comparison with Similar and Related Games}

\todo{Incubators very popular - you never know exactly what you're going to get, and some of the eggs contain very exciting Pokémon that are rarer in the wild. Also gives something to do in the game even if you can't pay attention to it or there is nothing interesting around.}

\todo{Gyms are the only point of interaction, and the only place to use any true amount of strategy. Also kind of the only in-game goal, as discussed in later sections, and gives rewards.}

\todo{Lures are social, efficient and can get some more interesting spawns}

\todo{Incense allows players to play in low-density areas, and in "dead zones" can spawn rare pokemon}

\todo{Shop is important for rural players with few pokestops (see more in section about quitting), and the only way to get additional lures, incense, incubators and lucky eggs. Talk about results of expenditure from follow-up survey.}

\todo{AR mode not very popular, probably because it makes it harder to catch things and uses additional battery. Would think it was popular only among beginning players, but is used by players from level 7 to 38}

\todo{What can we use in new games from the differences between pogo and other games? E.g. more story and immersion, ability to do some form of playing while stationary, more strategy}
...
Over 7 \% of players who quit the game found the game to be too repetitive, while almost 7 \% were less motivated to play because of the difficulty of progression. These two categories combined make up the more than 14 \% who complained about the \emph{grind} \advice{(I assume this term is common enough even outside gaming that I don't need to explain it?)} the game turned into. Starting at level 20, the number of experience points (\emph{XP}) needed to progress to the next level increases rapidly. Table \ref{tbl:experience-per-level} shows the required experience to reach each level along with the total accumulated experience so far for levels 18 and up. \advice{Could be a graph instead? Or both?}
